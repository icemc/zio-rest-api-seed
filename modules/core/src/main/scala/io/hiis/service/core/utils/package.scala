...
import zio.{ Tag, ZEnvironment, ZLayer }

package object utils {
  implicit final class ZIOConfigNarrowOps[R, E, A](
      private val self: ZLayer[R, E, A]
  ) extends AnyVal {

    /**
     * In bigger apps you can have a lot of components and, consequently - a lot of configuration
     * fields. It's not ideal to pass around the whole configuration object as a dependency for all
     * of those components: this way you break the separation of concerns principle. Component
     * should be aware only about dependencies it cares about and uses somehow.
     *
     * So to avoid that and do it in an ergonomic way, there's a narrow syntax extension for ZLayer,
     * available under import zio.config.syntax._. In the below example, we use both
     * zio-config-magnolia and zio-config-typesafe module inorder to have automatic derivation as
     * well as read/write HOCON structure.
     *
     * {{{
     *
     *  import zio._
